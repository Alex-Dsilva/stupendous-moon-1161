...
function Cart() {
  ...
  useEffect(() => {
    ...
    console.log("from page ", carts, checkedItems, carts.length)
    if (carts.length) {
      setCheckedItems(Array(carts.length || 0).fill(true))
      checkedItems.forEach((checked, index) => {
        if (checked && carts[index]) {
          console.log(index)
  ...
  return (
    <ChakraProvider>
      <Box maxW={{ sm: '700px', md: '1200px', lg: '1800px' }} m='auto' pt='2'>
        ...
        <Box h='70vh' w='98%' m='auto' >
          <Flex h='3em' mb='2' align={'center'} display={carts.length === 0 ? "none" : "flex"} bgColor={'#f1f1f1'} px='5' gap='5'>
            <Flex align={'center'}>
          ...
          {isLoading ? ("") : (
            <Flex flexDir={'column'} gap='5' h='100%' justifyContent={'center'} mb='10' >
              {carts && carts.length === 0 ? (
                <Flex flexDir="column" mb='5' align="center" gap="3">
                  <Image h='12rem' src="https://img.freepik.com/free-vector/man-shopping-supermarket_74855-7612.jpg?w=360&t=st=1686000535~exp=1686001135~hmac=266c051d123736bc5724e19d2c4cf6fd3953836f5dacadb85745a5c8a8698120" alt="Empty Cart" />
                  <Heading size="lg">Cart empty</Heading>
                  <Text>Find products to add to your Cart</Text>
                ...
                <Flex flexDir="column" gap="5" mb='10' >
                  {carts && carts.map((el, index) => {
                    return (
                      <Flex key={index + 1} flexDir={{ base: "column", lg: "row" }} h='fit-content' align={'center'} px='5' gap='5'>
          ...
          <Flex display={carts.length === 0 ? "none" : "flex"} flexDir={{ base: 'column-reverse', md: 'column', lg: "row" }} mb='5' borderRadius={'5px'} border='1.5px solid #c5c5c5' align={'center'} justify={'space-between'} gap='3' mt='5' p='2' py='8' h='fit-content'>
